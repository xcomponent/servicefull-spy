...
export type GlobalTransitionPropertiesAction =
  | ShowTransitionPropertiesAction
  | HideTransitionPropertiesAction
...
export const showTransitionProperties = (
  component: string,
  stateMachine: string,
  messageType: string,
  jsonMessageString: string
  ...
  return (dispatch: Dispatch<Action>, getState: () => XCSpyState) => {
    sessionXCSpy.PromiseCreateSession.then((session: Session) => {
      if (session.canSend(component, stateMachine, messageType)) {
        ...
        const privateTopic =
          getState().transitionProperties.defaultPrivateTopic;
        dispatch({
          ...
          privateTopic: privateTopic
            ? privateTopic
            : session.privateTopics.getDefaultPublisherTopic(),
...
export const setJsonMessageString = (
  jsonMessageString: string
): SetJsonMessageStringAction => {
  return {
